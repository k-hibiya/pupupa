...
/*------- ↓ SQL文を作成する -------*/
        ...
        if(isset($YorO)){
            ...
            if($initial == "あ" || $initial == "い" || $initial == "う" || $initial == "え" || $initial == "お" || 
                ...
                $sql = $sql."where $YorO like '$mojiset%' ";
                ...
                $sql = $sql."where $YorO like '$mojiset[0]%' 
                        or $YorO like '$mojiset[1]%' ";
            }else if($initial == "は" || $initial == "ひ" || $initial == "ふ" || $initial == "へ" || $initial == "ほ") {
                $sql = $sql."where $YorO like '$mojiset[0]%' 
                        or $YorO like '$mojiset[1]%' 
                        or $YorO like '$mojiset[2]%' ";
            }else if($initial == "すべて"){
                $sql = $sql;
            }
            if($sort == "posted_at") {
                ...
                if($YorO == "youjigo" && $sort == "asc") { //検索キーワードがようじ語・あいうえお順だったら
                    ...
                    <tr >
                        <td class="info" id="<?=$youjigo?><?=$main_id?>" colspan="2"> <!-- ← ページ内遷移のためのidを付けておく -->
                            ...
                            <span id="edit"><a href="youjigoEditAdmin.php?YorO=<?=$YorO?>&initial=<?=$initial?>&kodomo_id=<?=$kodomo_id?>&sort=<?=$sort?>&main_id=<?=$main_id?>&age=<?=$age?>">編集</a></span>
                        </td>
                    </tr> 
                    <tr >
